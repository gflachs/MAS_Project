...
public class AccountInfoService {
    ...
    public int getOrderAmount(String accountEmail) {
        Optional<AccountInfoEntity> optinal = accountInfoRepository.findByaccountEmail(accountEmail);
        if (optinal.isEmpty()) {
            return 0;
        }
        return optinal.get().getOrderEntities().size();
    }
